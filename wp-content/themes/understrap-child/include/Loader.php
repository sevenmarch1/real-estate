...
class Loader
{
    ...
    function setup(): self
    {
        ...
        // регистрируем страницы с опциями ACF
        if ($this->optionsPages && function_exists('acf_add_options_page')) {
            foreach ($this->optionsPages as $pageParams) {
                $pageParams['capability'] = $pageParams['capability'] ?? 'edit_posts';
                $pageParams['redirect'] = $pageParams['redirect'] ?? false;
                $pageParams['menu_title'] = $pageParams['menu_title'] ?? $pageParams['page_title'] ?? '';
                if (!empty($pageParams['parent_slug'])) {
                    acf_add_options_sub_page($pageParams);
                } else {
                    acf_add_options_page($pageParams);
                }
            }
        }
